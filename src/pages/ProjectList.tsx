...
import { Project, commands } from "../types";
...
export function ProjectList({
  ...
  const handleContextMenuAction = (key: React.Key) => {
    ...
    if (key === "openFolder") {
      commands.openPath(contextMenu.project.folder_path).catch((err) => {
  ...
  const getTitle = () => {
    ...
    if (viewMode === "recent") return "最近作品";
    return "作品集";
  };

  const getSubtitle = () => {
    const count = filteredProjects.length;
    if (viewMode === "favorites") return `共 ${count} 个收藏作品`;
    if (viewMode === "recent") return `最近 ${count} 个作品`;
    return `共 ${count} 本`;
  };
  ...
  return (
    <>
      ...
      <div 
        className="flex h-screen min-h-screen pt-9 gradient-bg"
      >
        {/* Sidebar */}
        <div 
        ...
        <main className="flex-1 flex flex-col h-full overflow-hidden relative">
          <div className="flex-1 overflow-y-auto p-8">
            <div className="max-w-6xl mx-auto">
              {/* Portfolio Header with Toggle and Search */}
              <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
                ...
                {/* View Mode Toggle + Search + New Project */}
                <div className="flex items-center gap-3">
                  {/* Search */}
                  <div
                    className={`flex items-center transition-all duration-300 ${isSearchVisible ? "w-48 opacity-100" : "w-0 opacity-0 overflow-hidden"}`}
                  >
                    <Input
                      placeholder="搜索作品..."
                      value={searchQuery}
                      onChange={(e) => setSearchQuery(e.target.value)}
                      className="bg-(--field-background)/50 backdrop-blur-sm rounded-full pl-4"
                    />
                  </div>
                  ...
                  <Button
                    ...
                    className={`rounded-full ${isSearchVisible ? "bg-(--novel-primary)/10 text-(--novel-primary)" : "text-(--novel-text-muted)"}`}
                  >
                    {Icons.search()}
                  </Button>
              ...
              {loading ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-8">
                  ...
                  <div
                    ...
                    onClick={onCreateProject}
                  >
                    <div className="aspect-3/4 rounded-[var(--radius)] border-2 border-dashed border-(--novel-border) flex flex-col items-center justify-center transition-all duration-300 group-hover:border-(--novel-primary) group-hover:bg-(--novel-primary)/5">
                      <div className="w-12 h-12 rounded-full bg-(--novel-primary)/10 text-(--novel-primary) flex items-center justify-center mb-3 transition-transform group-hover:scale-110">
                        <span className="text-2xl font-light">+</span>
                      </div>
                      <span className="text-sm text-(--novel-text-muted) group-hover:text-(--novel-primary) transition-colors">
                        新建作品
                      </span>
                    </div>
                  </div>
                  ...
                  {filteredProjects.map((project, index) => (
                    ...
                    >
                      {/* Card Container - Cover fills entire card */}
                      <div className="white-card mb-5 aspect-3/4 relative overflow-hidden">
                        {/* Favorite Badge */}
                        {project.is_favorite && (
                        ...
                        {/* Book Cover - Gradient Style - Fills entire card */}
                        <div className="absolute inset-0">
                          <div
                            className={`w-full h-full bg-gradient-to-br ${((): string => {
                              const saved = localStorage.getItem(`project_cover_${project.id}`);
                              const coverIdx = saved ? parseInt(saved) : index;
                              return coverStyles[coverIdx % coverStyles.length].gradient;
                            })()}`}
                          >
                            {/* Decorative elements */}
                            <div className="w-full h-full relative">
                              <div className="absolute top-3 right-3 w-8 h-12 border border-white/20 rounded-sm rotate-12" />
                              <div className="absolute bottom-4 left-3 w-10 h-8 border border-white/10 rounded-sm -rotate-6" />
                              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                                <div className="text-white/30 text-4xl font-serif italic">
                      ...
                      <div className="text-center px-1">
                        ...
                        >
                          ...
                          <span title="预计阅读时间">
                            {Math.max(1, Math.ceil(project.word_count / 250))} 分钟
                          </span>
                        </div>
                      </div>
                    </div>
        ...
        {deleteConfirmModal.isOpen && deleteConfirmModal.project && (
          <>
            <div
              className="fixed inset-0 z-50"
              style={{
              ...
              >
                ...
                >
                  ...
                  <span className="font-semibold">
                    {deleteConfirmModal.project.name}
                  </span>
                  》吗？删除后将无法恢复。
                </p>
                ...
                <div className="flex justify-end gap-3">
                  ...
                  <Button
                    ...
                    onPress={() => {
                      onDeleteProject(deleteConfirmModal.project!.path);
                      setDeleteConfirmModal({ isOpen: false, project: null });
                    }}
                  >
                    确定删除
